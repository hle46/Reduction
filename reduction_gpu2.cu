...
__global__ void getMin(float *input, size_t len, float *output_val,
  ...
  size_t i = tx + blockIdx.x * BLOCK_SIZE * 8;
  ...
  if (i < len) {
    ...
    if (i + 4 * BLOCK_SIZE < len) {
      a5 = input[i + 4 * BLOCK_SIZE];
      if (a5 < min_val) {
        min_val = a5;
        min_idx = i + 4 * BLOCK_SIZE;
      }
    }
    if (i + 5 * BLOCK_SIZE < len) {
      a6 = input[i + 5 * BLOCK_SIZE];
      if (a6 < min_val) {
        min_val = a6;
        min_idx = i + 5 * BLOCK_SIZE;
      }
    }
    if (i + 6 * BLOCK_SIZE < len) {
      a7 = input[i + 6 * BLOCK_SIZE];
      if (a7 < min_val) {
        min_val = a7;
        min_idx = i + 6 * BLOCK_SIZE;
      }
    }
    if (i + 7 * BLOCK_SIZE < len) {
      a8 = input[i + 7 * BLOCK_SIZE];
      if (a8 < min_val) {
        min_val = a8;
...
int main(int argc, char *argv[]) {
  ...
  begin = clock();
  size_t len_out = ceil((float)len / (BLOCK_SIZE * 8));
